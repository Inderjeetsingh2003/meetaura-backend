...
const signup=(async(req,res)=>
{
    ...
    try {
      ...
      if (user)
           {
              return res.status(200).json({success:0,message:"user already exists with this email"})
      }
          ...
          const username=name.toLowerCase().split(" ").join("")+Math.floor(1000 + Math.random() * 9000);
...
const logindirectly=(async(req,res)=>
{
    try{
        ...
        console.log("the  login is:", " ", email," ",password);
         ...
         if(!user)
            {
                return res.status(404).json({success:0,message:"user does not exits with this email"});
            }
